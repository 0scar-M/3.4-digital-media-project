...
public partial class GameNode : Node2D
{
	...
	public override void _Ready()
	{
		...
		GetNode<Label>("/root/GameScreen/GameUICanvas/GameUI/Controls/GoalMargin/GoalLabel").Text = $"Goal: {File.Goal}";
		
		// Add Camera Node
		AddChild((CameraNode) GD.Load<PackedScene>("res://Scenes/Game/CameraNode.tscn").Instantiate());
		Camera = GetNode<CameraNode>("CameraNode");
		Camera.Init(File, File.Camera, this);
		
		// CentreBody Node
		AddChild((CentreBodyNode) GD.Load<PackedScene>("res://Scenes/Game/CentreBodyNode.tscn").Instantiate());
		...
		{
			AddChild((BodyNode) GD.Load<PackedScene>("res://Scenes/Game/BodyNode.tscn").Instantiate());
		...
		AddChild((RocketNode) GD.Load<PackedScene>("res://Scenes/Game/RocketNode.tscn").Instantiate());
