...
public partial class BodyNode : BaseBodyNode
{
	...
	public void Init(LevelFile parsedFile, BodyFile parsedThisFile, GameNode parsedGame)
	{
		...
		*/
		...
		OrbitAngle = ThisFile.InitOrbitAngle;
		OrbitAngleSpeed = (float) Math.Sqrt(Game.GravityConst * Game.CentreBody.Mass / Math.Pow(OrbitRadius, 3));
		if (!ThisFile.IsOrbitClockwise)
			OrbitAngleSpeed *= -1;
		Position = Game.CentreBody.Position + _polarToCartesian(ThisFile.InitOrbitAngle - (float) Math.PI/2, OrbitRadius);
